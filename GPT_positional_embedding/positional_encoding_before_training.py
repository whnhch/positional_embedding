...
* Use CoLA datasete format
* We’ll also create an iterator for our dataset using the torch DataLoader class. This helps save on memory during training because, unlike a for loop, with an iterator the entire dataset does not need to be loaded into memory.
* For comfort, change string labels to index label

#### **Label List**

label lists are following

['org:member_of', 'per:schools_attended', 'per:charges', 'org:city_of_headquarters',
...
tr_gpt_input_ids, tr_gpt_attention_masks = get_gpt_token_ids(gpt_tokenizer, train_sentences, max_len)
dv_gpt_input_ids, dv_gpt_attention_masks = get_gpt_token_ids(gpt_tokenizer, dev_sentences, max_len)
ts_gpt_input_ids, ts_gpt_attention_masks = get_gpt_token_ids(gpt_tokenizer, test_sentences, max_len)
...
train_dataset = TensorDataset(tr_input_ids, tr_attribute_masks, tr_positional_ids, tr_seq_length, tr_gt_labels,
                              tr_gpt_input_ids, tr_gpt_attention_masks)
train_sampler = RandomSampler(train_dataset)
train_dataloader = DataLoader(train_dataset, sampler=train_sampler, batch_size=batch_size)

# torch.save(tr_input_ids, 'tr_input_ids.pt')
# torch.save(tr_attribute_masks, 'tr_attribute_masks.pt')
# torch.save(tr_positional_ids, 'tr_positional_ids.pt')
# torch.save(tr_seq_length, 'tr_seq_length.pt')
# torch.save(tr_gt_labels, 'tr_gt_labels.pt')
# torch.save(tr_gpt_input_ids, 'tr_gpt_input_ids.pt')
# torch.save(tr_gpt_attention_masks, 'tr_gpt_atttention_masks.pt')
...
dev_dataset = TensorDataset(dv_input_ids, dv_attribute_masks, dv_positional_ids, dv_seq_length, dv_gt_labels,
                            dv_gpt_input_ids, dv_gpt_attention_masks)
dev_sampler = SequentialSampler(dev_dataset)
dev_dataloader = DataLoader(dev_dataset, sampler=dev_sampler, batch_size=batch_size)

# torch.save(dv_input_ids, 'dv_input_ids.pt')
# torch.save(dv_attribute_masks, 'dv_attribute_masks.pt')
# torch.save(dv_positional_ids, 'dv_positional_ids.pt')
# torch.save(dv_seq_length, 'dv_seq_length.pt')
# torch.save(dv_gt_labels, 'dv_gt_labels.pt')
# torch.save(dv_gpt_input_ids, 'dv_gpt_input_ids.pt')
# torch.save(dv_gpt_attention_masks, 'dv_gpt_atttention_masks.pt')

"""## **3. Implement BERT+BiLSTM+MLP Model**

* Get bert contextual representaton, by BertModel
    * Use base-cased config, tokenizer and etc.
* Convert positional sequences to positional embeddings
    * Use torch.nn Embeddings, embed positional representation by **max_length x embedding_dim matrix**
        * max_length = 512
        * embedding_dim = 20
* Feed matrix to one layer Bidirectional LSTM
    * with hidden_size = 768
    * input shape is (max_length, batch_size, hidden_size)
    * output shape is (max_length, batch_size, hidden_size*2)
* Then, feed result of Bidirectional LSTM to one layer MLP
    * result of bidirecitonal LSTM should be reshaped to be classified at MLP
    * hidden size of MLP is 300
...
for epoch_i in range(epochs):
    ...
    for step, batch in enumerate(train_dataloader):
        ...
        b_gpt_input_ids = batch[5].to(device)
        b_gpt_attention_masks = batch[6].to(device)

        with torch.no_grad():
            token_probabilities = decoder(b_gpt_input_ids, b_gpt_attention_masks)

        # gpt_words = []
        # gpt_probs = []
        pos_probs = torch.empty((len(b_input_ids), max_len, max_len))
        prob_embeds = torch.empty((len(b_input_ids), 450, 768)).to(device)

        for i in range(batch_size):
            words = torch.empty((max_len))
            words_probs = torch.empty((max_len, max_len))
            words, words_probs = decoder.get_words_probs(gpt_tokenizer, b_gpt_input_ids[i], token_probabilities[i])
            # gpt_words.append(words)
            # gpt_probs.append(words_prob)
            pos_probs[i] = get_bert_token_probs(b_input_ids[i], words, words_probs, max_len)
            prob_embeds[i] = get_sinusoidal_embedding(max_len)
        ...
        prob_embeds = smart_sort(prob_embeds, indicies)
...
for batch in test_dataloader:
    ...
    b_input_ids, b_input_mask, b_input_positional_ids, b_input_seq_lengths, b_labels, b_gpt_input_ids, b_gpt_input_masks = batch

    with torch.no_grad():
        token_probabilities = decoder(b_gpt_input_ids, b_gpt_input_masks)
